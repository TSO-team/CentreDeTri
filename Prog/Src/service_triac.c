...
void service_triac_update(service_triac_Triac* triac) {
    ...
    float expectedTimeToActivate = triac->dutyCycle * SERVICE_TRIAC_EXPECTED_MAX_PULSE_DURATION_IN_MS;
    ...
    if(triac->timeFromPassageByZero < expectedTimeToActivate) {
        ...
        // (the commented if is a more robust implementation that activates the triac
        // for 1ms, and then deactivates it for 1ms, and repeats this until we pass by zero again)
        //if(triac->timeFromPassageByZero % 2) {
